...
function createServer(options = {}) {
  ...
  if (!password || password.length < 8) throw new Error('请设置至少 8 位的 ROOM_PASSWORD 入场密码');
  ...
  function identity(req) {
    ...
    const [id, expires, signature] = cookie.slice(15).split('.');
    if (!/^[a-f0-9]{48}$/.test(id || '') || !/^\d+$/.test(expires || '') || Number(expires) < now()) return null;
    if (!safeEq(sign(`${id}.${expires}`), signature || '')) return null;
    return id;
  }
  ...
  const heartbeat = setInterval(() => {
    for (const s of streams) { seen.set(s.id, now()); s.res.write(': heartbeat\n\n'); }
  ...
  const server = http.createServer(async(req,res) => {
    ...
    try {
      ...
      if (req.method !== 'GET') {
        ...
        if (origin && new URL(origin).host !== req.headers.host) fail('请在同一个网站内操作',403);
      ...
      if (req.method === 'POST' && url.pathname === '/api/login') {
        ...
        const id = identity(req) || token(), exp = String(now() + 90*86400000), value = `${id}.${exp}.${sign(`${id}.${exp}`)}`;
        seen.set(id,now()); json(res,200,{ok:true},{'Set-Cookie':`jinhua_session=${value}; Path=/; HttpOnly; SameSite=Lax; Max-Age=7776000${secureCookie ? '; Secure' : ''}`}); return;
      }
      const id = identity(req); if (!id) fail('请先输入入场密码',401); seen.set(id,now());
      ...
      if (req.method === 'GET' && op === 'events') {
        ...
        const stream={id,code,res}; streams.add(stream); req.on('close',()=>streams.delete(stream)); return;
      }
      ...
      if (op === 'join') {
        ...
        r.seats.push({id:token(),owner:id,name}); commit(r); json(res,200,payload(r,id)); return;
      }
