...
if(process.argv.includes('--verify-upload')){
  ...
  fs.writeFileSync(path.join(out,'release-notes.md'),`下载附件 **${archiveName}**，适用于群晖 DS923+ 的 Container Manager，含 AMD64 离线镜像、源码、配置与安装说明。\n\n- 修复全押加注后提前结算的问题，保留其他玩家回应机会。\n- 完善合法全押选项及二次确认，明确本次投入金额。\n- 优化手机大厅、长昵称、弹窗、密码显示和日志阅读体验。\n- 超时存档失败时拒绝迟到行动；收紧卡牌编码和动作校验。\n- 83 项自动测试通过，并完成窄屏及桌面浏览器操作验证。\n\n完整测试、Linux Docker 启动、健康检查和登录验证通过：[查看发布验证](${runURL})。尚未在群晖或真实手机上完成本轮验证。\n\n升级前备份并保留 data 和 .env，保持原端口，导入 jinhua-friends:${version} 后重新创建容器。请先结束正在进行的牌局；v1.2.1 升级无需手工迁移存档，新增回应状态在下一局初始化。反向代理配置单独使用，详见 README 与 SECURITY.md。\n\n本安装包由 GitHub Actions 构建；请使用本 Release 的 SHA256SUMS.txt 校验 ZIP，镜像校验值在 ZIP 内。\n`);
